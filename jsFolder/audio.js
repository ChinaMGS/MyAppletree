...
Tools.serverRequest(
	...
	function(data){
		...
		if(oData.code == '1'){
			...
			// 	musicArr.push(v.playUrl64);
			// });
			musicArr = oData.tracks.list;
			currentArr = musicArr.slice(0);
			console.log(musicArr);
			console.log(currentArr);
			Audio.init( _$audio, 0 , currentArr);
		}
	}
);
...
var Audio = {
	 ...
	 * @return {[type]}       [description]
	 */
	init: function(audio, i , obj){
		i = arguments[1]?arguments[1]:0;
		if(obj.length<1){
			return false;
		}else{
			audio.attr('src', obj[i].playUrl64);
			audio[0].play();
			currentIndex = i;
			Audio.showTime(audio);
			_$opBtn.removeClass('icon-play').addClass('icon-pause');
			$('.js-name').html(obj[i].title);
			$('.js-nick').html(obj[i].nickname);
			var listLayer = baidu.template("js-tem-layer");
			var data = {};
			data.obj = obj;
			el = listLayer(data);
		...
		audio.on('timeupdate',function(e){
			...
			if(!isNaN(that.duration)){
	            ...
	            if(progressValue >= 9990){
	            	...
	            	Audio.nextSong(audio , currentArr);
	            }
	...
	nextSong: function(ev , arr){
		var audio = ev.data.audio?ev.data.audio:arguments[0];
		var arr = ev.data.arr?ev.data.arr:arguments[1];
		var len = arr.length;
		if(currentIndex<len-1){
			currentIndex++;
			Audio.init( audio, currentIndex , currentArr);
		}else{
			currentIndex = currentIndex + 1 - len;
			Audio.init( audio, currentIndex , currentArr);
		}
	},
	prevSong: function(ev , arr){
		var audio = ev.data.audio?ev.data.audio:arguments[0];
		var arr = ev.data.arr?ev.data.arr:arguments[1];
		var len = arr.length;
		if(currentIndex>0){
...
//点击上一曲
$('.js-prev').on('touchend',function(){
	Audio.prevSong(_$audio,currentArr);
})

//点击下一曲
$('.js-next').on('touchend',function(){
	Audio.nextSong(_$audio,currentArr);
})
...
$('.js-menu').on('click',function(){
	layer.open({
	    ...
	    ,content: el
	    ,anim: 'up'
	    ,style: 'position:absolute;bottom:0.28rem;left:50%;margin-left:-4.78rem;border-radius: 0.133333rem;background:rgba(255,255,255,0.6);width:9.56rem;'
	    ,success: function(){
	    	$('.js-close').on('click',function(){
	    		layer.closeAll();
	    	})
	    	$('.js-menu-play').on('click',function(){
	    		if(!$(this).hasClass('active')){
	    			$(this).addClass('active').siblings().removeClass('active');
	    			Audio.init( _$audio, $(this).attr('data-index'), currentArr);
	    			layer.closeAll();
	    		}
	    	})
	    }
